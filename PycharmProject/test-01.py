...
class PPPoEServer:
    ...
    def send_ipcp_rej_packet(self, pkt):
        ...
        _pkt = Ether(src=pkt.dst, dst=pkt.src, type=0x8864) / PPPoED(version=1, type=1, code=0x00,
                                                                     sessionid=SESSION_ID) / PPP(
            proto=0x8021) / _payload
        scapy.sendp(_pkt)
    ...
    def send_lcp_echo_request(self, pkt):
        ...
        lcp_req = Ether(src=pkt.dst, dst=pkt.src, type=0x8864) / PPPoED(version=1, type=1, code=0x00,
                                                                        sessionid=SESSION_ID) / PPP(
            proto=0xc021) / _payload
        scapy.sendp(lcp_req)
    ...
    def send_lcp_end_packet(self, pkt):
        ...
        _pkt = Ether(src=pkt.dst, dst=pkt.src, type=0x8864) / PPPoE(version=0x1, type=0x1, code=0x00,
                                                                    sessionid=SESSION_ID) / PPP(proto=0xc021) / _payload
        scapy.sendp(_pkt)
    ...
    def send_pads_packet(self, pkt):
        ...
        sendpkt = Ether(src=MAC_ADDRESS, dst=pkt.src, type=0x8863) / PPPoED(version=1, type=1, code=0x65,
                                                                            sessionid=pkt.sessionid,
                                                                            len=len(_payload)) / _payload
        scapy.sendp(sendpkt)
    ...
    def send_pado_packet(self, pkt):
        ...
        sendpkt = Ether(src=MAC_ADDRESS, dst=pkt.src, type=0x8863) / PPPoED(version=1, type=1, code=0x07,
                                                                            sessionid=pkt.sessionid,
                                                                            len=len(_payload)) / _payload
        scapy.sendp(sendpkt)
